...
class Miruro : Source() {
    ...
    // [preferences] and [handler] are supplied by `extensions.utils.Source`.

    // ────────────────────────────── 5-day trial ──────────────────────────────
    //
    // This build ships with a 5-day trial that starts on [TRIAL_START_TIMESTAMP].
    // While the trial is active [baseUrl] resolves to the user's preferred Miruro
    // mirror and every feature behaves normally. Once [TRIAL_EXPIRATION_TIMESTAMP]
    // passes, [baseUrl] switches to [EXPIRED_BASE_URL] and every entry point
    // short-circuits into a single "Trial Period Expired" placeholder entry — see
    // [trialExpiredAnime] / [trialExpiredPage]. [EXPIRED_BASE_URL] is a real,
    // reachable URL, so the request hooks that the app drives itself
    // ([animeDetailsRequest], [episodeListRequest]) still complete successfully and
    // can then return the expired placeholder from their parse step instead of
    // throwing a network error at the user.
    private val isTrialExpired: Boolean
        get() = System.currentTimeMillis() >= TRIAL_EXPIRATION_TIMESTAMP

    override var baseUrl by LazyMutable {
        if (isTrialExpired) EXPIRED_BASE_URL else preferences.preferredMirror
    }
        ...
        mirrorFetchJob = scope.launch(
            ...
            try {
                ...
                if (currentMirror !in values) {
                    ...
                    // After the trial expires `baseUrl` stays pinned to EXPIRED_BASE_URL;
                    // only the stored preference is refreshed here.
                    if (!isTrialExpired) baseUrl = newDefault
                }
    ...
    companion object {
        ...
        // ───────────────────────────── 5-day trial ─────────────────────────────
        // Trial window: 2026-09-21 00:00 (+06) → 2026-09-26 00:00 (+06).
        // After expiry every entry point serves [TRIAL_EXPIRED_TITLE] and
        // [baseUrl] resolves to [EXPIRED_BASE_URL].
        private const val TRIAL_START_TIMESTAMP = 1789927200000L
        ...
        internal const val EXPIRED_BASE_URL = "https://cdn.dribbble.com/userupload/23193562/file/original-01678fc58a3e26f5817db853432cd78a.png?resize=752x&vertical=center"
        private const val TRIAL_EXPIRED_TITLE = "Trial Period Expired"
        private const val TRIAL_EXPIRED_DESCRIPTION = "The 5-day trial period has ended.\n\nThe base URL is now set to the expiry notice image. Reinstall an updated build of this extension to restore Miruro.tv browsing."
    ...
    // Dedicated client for `status.miruro.com` only; the rate limit is scoped to
    // this client rather than the shared one, so one request per two seconds is
    // equivalent to a per-host rule here.
    private val statusPageClient = network.client.newBuilder()
        .rateLimit(permits = 1, period = 2.seconds)
        .build()
    ...
    /**
     * The single placeholder entry shown once the 5-day trial ends. Its
     * thumbnail is the expiry notice image and its URL is left blank so the app
     * never tries to fetch a Miruro page for it.
     */
    private fun trialExpiredAnime(): SAnime = SAnime.create().apply {
        title = TRIAL_EXPIRED_TITLE
        thumbnail_url = EXPIRED_BASE_URL
        description = TRIAL_EXPIRED_DESCRIPTION
        status = SAnime.COMPLETED
        url = ""
        initialized = true
    }

    private fun trialExpiredPage(): AnimesPage = AnimesPage(listOf(trialExpiredAnime()), false)
    ...
    override fun setupPreferenceScreen(screen: PreferenceScreen) {
        ...
        val mirrorPref = screen.getListPreference(
            ...
            onChange = { pref, value ->
                ...
                // The preference itself is persisted by the ListPreference; keep the
                // in-memory [baseUrl] pinned to EXPIRED_BASE_URL once the trial ends.
                if (!isTrialExpired) baseUrl = value
                configCache = ProviderConfigCache(
                    displayNames = KNOWN_DISPLAY_NAMES,
                    entries = DEFAULT_PROVIDER_ENTRIES,
                    values = DEFAULT_PROVIDER_VALUES,
