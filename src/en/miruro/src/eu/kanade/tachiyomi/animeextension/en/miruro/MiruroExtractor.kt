...
class MiruroExtractor(
    ...
    companion object {
         ...
         * / ChillX family). Pre-routed to their dedicated extractors instead of
         * the generic WebView sniffer, which cannot decrypt their obfuscated
         * player payloads and would return an empty list.
         */
    ...
    /**
     * Generic WebView-based resolver for the long tail of `type == "embed"`
     * streams. Miruro pre-routes the two Zoro-family hosts (MegaCloud /
     * RapidCloud) to their dedicated extractors — see [extractPreRoutedEmbed] —
     * and everything else is sniffed through a headless [UniversalExtractor],
     * which covers the many one-off hosts without a per-host implementation.
     */
    private val embedExtractor by lazy { UniversalExtractor(client) }
    ...
    private fun extractPreRoutedEmbed(
        ...
        return when {
            ...
            RAPID_CLOUD_HOSTS.any { lowerHost.contains(it) } -> runCatching {
                ...
                // UniversalExtractor cannot attach external subtitle tracks itself,
                // so the source-level [subtitles] list is re-attached here.
                embedExtractor.videosFromUrl(embedUrl, headers, prefix = qualityLabel)
                    .map { video ->
                        if (video.subtitleTracks.isEmpty()) video.copy(subtitleTracks = subtitles) else video
                    }
            }.onFailure {
                Log.w(TAG, "Generic embed extraction failed for ${embedUrl.take(80)}: ${it.message}")
            }.getOrDefault(emptyList())
        }
    }
}
