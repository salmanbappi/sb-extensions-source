...
class MoviesMod : Source() {
    ...
    override suspend fun getVideoList(episode: SEpisode): List<Video> {
        val urlJson = runCatching { json.decodeFromString<EpLinks>(episode.url) }.getOrNull()
            ?: return emptyList()
            ...
            when {
                href.contains("cdn.video-gen.xyz") || href.contains("video-seed.dev") ||
                    ...
                    if (finalUrl.contains(".m3u8")) {
                        ...
                        }.getOrDefault(listOf(Video(videoUrl = finalUrl, videoTitle = "$quality - HLS$size")))
                    } else {
    ...
    private fun getDomainPrefSummary(): String = (preferences.getString(PREF_DOMAIN_KEY, PREF_DOMAIN_DEFAULT) ?: PREF_DOMAIN_DEFAULT).let {
        """$it
