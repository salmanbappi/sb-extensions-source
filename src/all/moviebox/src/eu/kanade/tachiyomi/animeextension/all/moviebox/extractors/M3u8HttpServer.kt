// ============================================================================
// Vendored from lib/m3u8server/M3u8HttpServer.kt via `cli.py vendor`
...
class M3u8HttpServer(
    ...
    private fun handleM3u8Request(session: IHTTPSession): Response {
        ...
        val fallbackCookie = session.parameters["cookie"]?.first()
        val headers = extractHeadersFromSession(session, fallbackReferer, fallbackUserAgent, fallbackCookie)
    ...
    private fun extractHeadersFromSession(
        ...
        if (!fallbackCookie.isNullOrBlank()) {
            extractedHeaders["cookie"] = fallbackCookie
        }

        Log.d(tag, "Extracted headers (referer=${extractedHeaders["referer"]?.take(80) ?: "none"}, ua=${extractedHeaders["user-agent"]?.take(40) ?: "none"}, cookie=${if (extractedHeaders["cookie"].isNullOrEmpty()) "none" else "present"})")
    ...
    private suspend fun processM3u8Content(url: String, headers: Map<String, String> = emptyMap()): String = withContext(Dispatchers.IO) {
        try {
            ...
            val modifiedContent = modifyM3u8Content(m3u8Content, url, port, referer, userAgent, cookie)
    ...
    fun createLocalUrl(
        ...
        if (!cookie.isNullOrBlank()) {
            sb.append("&cookie=").append(URLEncoder.encode(cookie, Charsets.UTF_8.name()))
        }
        return sb.toString()
    }
            ...
            when {
                ...
                line.startsWith("#") || line.isBlank() -> {
                    ...
                    if (resolvedUrl.contains(".m3u8", ignoreCase = true)) {
                        ...
                        modifiedLines.add(buildChildM3u8Url(serverPort, resolvedUrl, referer, userAgent, cookie))
                    } else {
                        modifiedLines.add(
                            createLocalSegmentUrl(serverPort, resolvedUrl, currentKey, referer, userAgent, cookie),
                        )
                        segmentCount++
                    }
                    segmentSequence++
    ...
    private fun createLocalSegmentUrl(
        ...
        return buildString {
            ...
            if (!cookie.isNullOrBlank()) {
                append("&cookie=")
                append(URLEncoder.encode(cookie, Charsets.UTF_8.name()))
            }
        }
    }
}
