// ============================================================================
// Vendored from lib/m3u8server/M3u8Integration.kt via `cli.py vendor`
...
class M3u8Integration(
     ...
     * `Cookie`, which are then re-encoded into the proxied URL so the m3u8
     ...
     *
     * The Cookie leg matters for CDNs gated by a signed edge-cache cookie
     * (e.g. MovieBox `sbcdn2.hakunaymatata.com` streams, whose `signCookie`
     * from the play-info API is required on every playlist and segment
     * request — dropping it yields HTTP 403 ACCESS DENIED).
     */
    private fun processM3u8Video(originalVideo: Video): Video {
        ...
        val cookie = originalVideo.headers?.get("Cookie")
        val processedUrl = serverManager.processM3u8Url(originalVideo.videoUrl, referer, userAgent, cookie)
        return Video(
