// ============================================================================
// Vendored from lib/m3u8server/M3u8ServerManager.kt via `cli.py vendor`
...
class M3u8ServerManager(
     ...
     * @param cookie optional Cookie to encode alongside the referer — required
     *   by signed-edge-cache CDNs (e.g. `sbcdn2.hakunaymatata.com`) that
     *   answer 403 ACCESS DENIED when the sign cookie is missing.
    ...
    fun processM3u8Url(
        ...
        cookie: String? = null,
    ): String? = server?.createLocalUrl(m3u8Url, referer, userAgent, cookie)
